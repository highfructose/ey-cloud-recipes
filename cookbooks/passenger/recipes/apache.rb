...
node[:applications].each_with_index do |(app, data), count|
  ...
  if data[:vhosts][1]
    ...
    update_file "#{app}-ssl-key" do
      action :rewrite

      path "/data/apache2/ssl/#{app}.key"
      body data[:vhosts][1][:key]
    end

    update_file "#{app}-ssl-cert" do
      action :rewrite

      path "/data/apache2/ssl/#{app}.crt"
      body data[:vhosts][1][:crt]
    end

    update_file "#{app}-ssl-chain" do
      action :rewrite

      path "/data/apache2/ssl/#{app}.chain"
      body data[:vhosts][1][:chain]
    end
